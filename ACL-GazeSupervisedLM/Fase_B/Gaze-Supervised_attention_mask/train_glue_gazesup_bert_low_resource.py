...
from measured_scanpath_utils import (
	...
	compute_gaze_feature_norm_stats,
	extract_measured_gaze_features,
	infer_custom_label_schema,
	load_measured_scanpath_dataset,
	load_gaze_feature_norm_stats,
	measured_gaze_alignment_ok,
	normalize_scanpath_source,
	parse_gaze_feature_names,
	save_json,
)
...
VARIANTE_A_ROOT = Path("Variante_A_metricas_rama_auxiliar/Pasos")


def ensure_variante_a_dirs():
	for step_name in (
		"paso_A_dataset_metricas",
		"paso_B_batch_collator",
		"paso_C_model_fusion",
		"paso_D_alignment_subtokens",
	):
		(VARIANTE_A_ROOT / step_name).mkdir(parents=True, exist_ok=True)
...
class ModelArguments:
	...
	use_gaze_features: bool = field(
		default=False,
		metadata={"help": "If True, fuse measured fixation features into the auxiliary scanpath branch before the GRU."},
	)
	gaze_feature_dim: int = field(
		default=3,
		metadata={"help": "Number of measured gaze features per fixation."},
	)
	gaze_feature_names: str = field(
		default="FFD,TRT,nFix",
		metadata={"help": "Comma-separated fixation features to use. TRT falls back to TFD; nFix falls back to FC."},
	)
	gaze_feature_norm_path: Optional[str] = field(
		default=None,
		metadata={"help": "Optional JSON file with train mean/std for measured gaze features."},
	)
	debug_gaze_features: bool = field(
		default=False,
		metadata={"help": "If True, write small Variante A debug files for dataset, batch and model fusion shapes."},
	)
...
def main():
	...
	requested_gaze_feature_names = parse_gaze_feature_names(model_args.gaze_feature_names)
	if model_args.gaze_feature_dim != len(requested_gaze_feature_names):
		raise ValueError(
			f"--gaze_feature_dim ({model_args.gaze_feature_dim}) must match --gaze_feature_names "
			f"({len(requested_gaze_feature_names)} names)."
		)
	if model_args.use_gaze_features:
		ensure_variante_a_dirs()
	...
	gaze_feature_norm_stats = None
	gaze_feature_mapping_for_debug = None
	gaze_debug_counters = {
		"nan_inf_replaced": 0,
		"mismatches_discarded": 0,
		"sample_examples_written": 0,
		"feature_names_seen": set(),
	}
	if is_custom_measured_task and model_args.scanpath_source == "measured" and model_args.use_gaze_features:
		paso_a_dir = VARIANTE_A_ROOT / "paso_A_dataset_metricas"
		mismatch_path = paso_a_dir / "mismatches.jsonl"
		mismatch_path.write_text("", encoding="utf-8")
		(paso_a_dir / "sample_examples.jsonl").write_text("", encoding="utf-8")

		def keep_gaze_aligned_example(example, idx=None):
			ok = measured_gaze_alignment_ok(example)
			if not ok:
				gaze_debug_counters["mismatches_discarded"] += 1
				append_jsonl(mismatch_path, {
					"index": idx,
					"trial_id": example.get("trial_id"),
					"word_id_len": len(example.get("word_id") or []),
					"reading_features_by_fixation_len": len(example.get("reading_features_by_fixation") or []),
				})
			return ok

		raw_datasets = raw_datasets.filter(
			keep_gaze_aligned_example,
			with_indices=True,
			load_from_cache_file=False,
			desc="Filtering measured gaze feature length mismatches",
		)

		if model_args.gaze_feature_norm_path and os.path.exists(model_args.gaze_feature_norm_path):
			gaze_feature_norm_stats = load_gaze_feature_norm_stats(model_args.gaze_feature_norm_path)
		else:
			gaze_feature_norm_stats = compute_gaze_feature_norm_stats(
				raw_datasets["train"],
				requested_feature_names=requested_gaze_feature_names,
			)
			model_args.gaze_feature_norm_path = str(paso_a_dir / "gaze_feature_norm.json")
			save_json(model_args.gaze_feature_norm_path, gaze_feature_norm_stats)

		mappings_seen = gaze_feature_norm_stats.get("mappings_seen") or []
		gaze_feature_mapping_for_debug = mappings_seen[0] if mappings_seen else {}
		save_json(str(paso_a_dir / "gaze_feature_mapping.json"), {
			"requested_features": requested_gaze_feature_names,
			"mapping": gaze_feature_mapping_for_debug,
			"all_mappings_seen": mappings_seen,
		})
		save_json(str(paso_a_dir / "dataset_metricas_summary.json"), {
			"measured_scanpath_file": data_args.measured_scanpath_file,
			"requested_features": requested_gaze_feature_names,
			"feature_names_seen": gaze_feature_norm_stats.get("feature_names_seen", []),
			"mapping": gaze_feature_mapping_for_debug,
			"normalization_path": model_args.gaze_feature_norm_path,
			"nan_inf_replaced_while_estimating_stats": gaze_feature_norm_stats.get("replaced_nonfinite", 0),
			"mismatches_discarded": gaze_debug_counters["mismatches_discarded"],
		})
		logger.warning(f"Variante A feature_names encontrados: {gaze_feature_norm_stats.get('feature_names_seen', [])}")
		logger.warning(f"Variante A mapeo usado: {gaze_feature_mapping_for_debug}")
	...
	def preprocess_function(examples):
		...
		if is_custom_measured_task and model_args.scanpath_source == "measured":
			...
			if model_args.use_gaze_features:
				features["measured_gaze_features"] = []
				features["measured_gaze_feature_mask"] = []
			...
			for idx in range(total):
				...
				feature_names = examples["feature_names"][idx] if model_args.use_gaze_features and "feature_names" in examples else None
				reading_features_by_fixation = examples["reading_features_by_fixation"][idx] if model_args.use_gaze_features and "reading_features_by_fixation" in examples else None
				reading_features_mask_by_fixation = examples["reading_features_mask_by_fixation"][idx] if model_args.use_gaze_features and "reading_features_mask_by_fixation" in examples else None
				measured_feature = build_measured_single_sentence_features(
					...
					requested_gaze_feature_names=requested_gaze_feature_names,
					gaze_feature_norm_stats=gaze_feature_norm_stats,
					use_gaze_features=model_args.use_gaze_features,
				)
				for key, value in measured_feature.items():
					features[key].append(value)
				if model_args.use_gaze_features:
					extracted_raw = extract_measured_gaze_features(
					...
					gaze_debug_counters["nan_inf_replaced"] += extracted_raw["replaced_nonfinite"]
					gaze_debug_counters["feature_names_seen"].update(feature_names or [])
					if gaze_debug_counters["sample_examples_written"] < 5:
						sample_payload = {
							"trial_id": examples["trial_id"][idx] if "trial_id" in examples else None,
							"text": examples[sentence1_key][idx],
							"feature_names": feature_names,
							"mapping": extracted_raw["mapping"],
							"word_id_len": len(examples[data_args.measured_word_id_field][idx]),
							"measured_gaze_features_shape": [
								len(measured_feature["measured_gaze_features"][0]),
								len(measured_feature["measured_gaze_features"][0][0]) if measured_feature["measured_gaze_features"][0] else 0,
							],
							"measured_gaze_features_first_rows": measured_feature["measured_gaze_features"][0][:2],
						}
						append_jsonl(VARIANTE_A_ROOT / "paso_A_dataset_metricas" / "sample_examples.jsonl", sample_payload)
						gaze_debug_counters["sample_examples_written"] += 1
	...
	if is_custom_measured_task and model_args.scanpath_source == "measured" and model_args.use_gaze_features:
		save_json(str(VARIANTE_A_ROOT / "paso_A_dataset_metricas" / "dataset_metricas_summary.json"), {
			"measured_scanpath_file": data_args.measured_scanpath_file,
			"requested_features": requested_gaze_feature_names,
			"feature_names_seen": sorted(gaze_debug_counters["feature_names_seen"]),
			"mapping": gaze_feature_mapping_for_debug,
			"normalization_path": model_args.gaze_feature_norm_path,
			"nan_inf_replaced": gaze_debug_counters["nan_inf_replaced"],
			"mismatches_discarded": gaze_debug_counters["mismatches_discarded"],
			"sample_examples_written": gaze_debug_counters["sample_examples_written"],
		})
	...
	class OurDataCollatorWithPadding:
		...
		def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
			...
			bs = len(features)
			write_batch_debug = model_args.use_gaze_features and model_args.debug_gaze_features and not hasattr(self, "_wrote_variante_a_batch_debug")
			...
			if "measured_gaze_features" in features[0]:
				gaze_sequences = []
				gaze_masks = []
				max_gaze_length = 0
				gaze_feature_dim = len(features[0]["measured_gaze_features"][0][0]) if features[0]["measured_gaze_features"][0] else model_args.gaze_feature_dim
				for feature in features:
					for i in range(num_sent):
						current_features = feature["measured_gaze_features"][i]
						current_mask = feature.get("measured_gaze_feature_mask", [[[]]])[i]
						gaze_sequences.append(current_features)
						gaze_masks.append(current_mask)
						max_gaze_length = max(max_gaze_length, len(current_features))

				gaze_tensor = torch.zeros((bs * num_sent, max_gaze_length, gaze_feature_dim), dtype=torch.float32)
				gaze_mask_tensor = torch.zeros((bs * num_sent, max_gaze_length, gaze_feature_dim), dtype=torch.float32)
				for row_index, current_features in enumerate(gaze_sequences):
					if len(current_features) == 0:
						continue
					valid_len = min(len(current_features), max_gaze_length)
					gaze_tensor[row_index, :valid_len, :] = torch.tensor(current_features[:valid_len], dtype=torch.float32)
					if row_index < len(gaze_masks) and len(gaze_masks[row_index]) > 0:
						gaze_mask_tensor[row_index, :valid_len, :] = torch.tensor(gaze_masks[row_index][:valid_len], dtype=torch.float32)

				batch["measured_gaze_features"] = gaze_tensor.view(bs, num_sent, max_gaze_length, gaze_feature_dim)
				batch["measured_gaze_feature_mask"] = gaze_mask_tensor.view(bs, num_sent, max_gaze_length, gaze_feature_dim)

				if write_batch_debug:
					paso_b_dir = VARIANTE_A_ROOT / "paso_B_batch_collator"
					paso_b_dir.mkdir(parents=True, exist_ok=True)
					shapes_lines = [
						f"batch_size: {bs}",
						f"num_sent: {num_sent}",
						f"measured_word_ids.shape: {tuple(batch['measured_word_ids'].shape)}",
						f"measured_sp_len.shape: {tuple(batch['measured_sp_len'].shape)}",
						f"measured_gaze_features.shape: {tuple(batch['measured_gaze_features'].shape)}",
						f"measured_gaze_feature_mask.shape: {tuple(batch['measured_gaze_feature_mask'].shape)}",
					]
					(paso_b_dir / "batch_shapes.txt").write_text("\n".join(shapes_lines) + "\n", encoding="utf-8")
					save_json(str(paso_b_dir / "sample_batch_summary.json"), {
						"batch_size": bs,
						"max_scanpath_len": max_gaze_length,
						"measured_word_ids.shape": list(batch["measured_word_ids"].shape),
						"measured_sp_len.shape": list(batch["measured_sp_len"].shape),
						"measured_gaze_features.shape": list(batch["measured_gaze_features"].shape),
						"first_example_features_before_padding": gaze_sequences[0][:2] if gaze_sequences else [],
						"first_example_features_after_padding": batch["measured_gaze_features"][0, 0, :2, :].tolist(),
					})
					self._wrote_variante_a_batch_debug = True
