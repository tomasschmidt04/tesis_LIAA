...
class GazeFeatureFusion(nn.Module):
	def __init__(self, hidden_size, gaze_feature_dim=3, dropout=0.1):
		super().__init__()
		self.gaze_projection = nn.Linear(gaze_feature_dim, hidden_size)
		self.gaze_gate = nn.Linear(2 * hidden_size, hidden_size)
		self.layer_norm = nn.LayerNorm(hidden_size)
		self.dropout = nn.Dropout(dropout)
		nn.init.constant_(self.gaze_gate.bias, -2.0)

	def forward(self, x_sp, measured_gaze_features, return_debug_dict=False):
		gaze_proj = self.gaze_projection(measured_gaze_features)
		gaze_proj = self.dropout(gaze_proj)
		fusion_input = torch.cat([x_sp, gaze_proj], dim=-1)
		gate = torch.sigmoid(self.gaze_gate(fusion_input))
		x_fused = self.layer_norm(x_sp + gate * gaze_proj)
		if not return_debug_dict:
			return x_fused, None
		return x_fused, {
			"gaze_proj_shape": tuple(gaze_proj.shape),
			"gate_mean": float(gate.detach().mean().cpu().item()),
			"gate_min": float(gate.detach().min().cpu().item()),
			"gate_max": float(gate.detach().max().cpu().item()),
		}
...
class SP_Encoder(nn.Module):
	...
	def __init__(self, config, scanpath_source="eyettention", use_gaze_features=False, gaze_feature_dim=3, debug_gaze_features=False):
		super().__init__()
		self.scanpath_source = (scanpath_source or "eyettention").lower()
		self.use_gaze_features = bool(use_gaze_features)
		self.gaze_feature_dim = int(gaze_feature_dim)
		self.debug_gaze_features = bool(debug_gaze_features)
		self._wrote_gaze_fusion_debug = False
		...
		self.gaze_feature_fusion = GazeFeatureFusion(
			hidden_size=config.hidden_size,
			gaze_feature_dim=self.gaze_feature_dim,
			dropout=0.1,
		) if self.use_gaze_features else None
	...
	def convert_word_pos_seq_to_token_pos_seq(self,
												...
												sn_len,
												word_ids_sn,
												measured_gaze_features=None,
												measured_gaze_feature_mask=None,
												):
		...
		for b in range(word_pos.shape[0]):
			...
			if num_sent == 2:
				...
				valid_pos_seq = torch.cat((valid_pos_seq, SEP_indx[b].reshape(1))) #add SEP token back to differentiate two sentences

				valid_pos_seq_2 = torch.masked_select(word_pos_2[b,:], stop_mask_2[b,:])[1:] + SEP_indx[b]
			...
			cur_gaze_features = []
			for fixation_index, p in enumerate(valid_pos_seq):
				idx = torch.where(word_ids_sn[b]==p)[0]
				if measured_gaze_features is not None:
					if fixation_index < measured_gaze_features.shape[1]:
						fixation_features = measured_gaze_features[b, fixation_index]
					else:
						fixation_features = torch.zeros(
							measured_gaze_features.shape[-1],
							dtype=measured_gaze_features.dtype,
							device=measured_gaze_features.device,
						)
				for i in idx:
					cur_token_pos = torch.cat((cur_token_pos, (p + word_ids_2_token_ids_sn[b][i]).reshape(1)))
					if measured_gaze_features is not None:
						cur_gaze_features.append(fixation_features)

			gaze_token_pos.append(cur_token_pos[1:]) # remove the fake CLS token
			if measured_gaze_features is not None:
				if len(cur_gaze_features) == 0:
					gaze_features_token_level.append(torch.zeros(
						(0, measured_gaze_features.shape[-1]),
						dtype=measured_gaze_features.dtype,
						device=measured_gaze_features.device,
					))
				else:
					gaze_features_token_level.append(torch.stack(cur_gaze_features, dim=0))
		...
		for indx in torch.where(sp_len==0)[0]:
			...
			if measured_gaze_features is not None:
				gaze_features_token_level[indx] = torch.zeros(
					(1, measured_gaze_features.shape[-1]),
					dtype=measured_gaze_features.dtype,
					device=measured_gaze_features.device,
				)
			sp_len[indx] = 1
		...
		if measured_gaze_features is not None:
			gaze_features_token_level = pad_sequence(gaze_features_token_level, batch_first=True, padding_value=0.0)
			if gaze_features_token_level.shape[1] != gaze_token_pos.shape[1]:
				alignment_log = Path("Variante_A_metricas_rama_auxiliar/Pasos/paso_D_alignment_subtokens/alignment_warnings.jsonl")
				alignment_log.parent.mkdir(parents=True, exist_ok=True)
				with alignment_log.open("a", encoding="utf-8") as handle:
					handle.write(json.dumps({
						"warning": "token_level_feature_length_mismatch",
						"gaze_token_pos_shape": tuple(gaze_token_pos.shape),
						"gaze_features_token_level_shape": tuple(gaze_features_token_level.shape),
					}) + "\n")
				target_len = gaze_token_pos.shape[1]
				if gaze_features_token_level.shape[1] > target_len:
					gaze_features_token_level = gaze_features_token_level[:, :target_len, :]
				else:
					pad_len = target_len - gaze_features_token_level.shape[1]
					padding = torch.zeros(
						(gaze_features_token_level.shape[0], pad_len, gaze_features_token_level.shape[-1]),
						dtype=gaze_features_token_level.dtype,
						device=gaze_features_token_level.device,
					)
					gaze_features_token_level = torch.cat([gaze_features_token_level, padding], dim=1)
			return gaze_token_pos, sp_len, gaze_features_token_level

		return gaze_token_pos, sp_len

	def SP_Gen(self, input_ids, attention_mask, token_type_ids, word_ids, word_len, LM_word_ids, measured_word_ids=None, measured_sp_len=None, measured_gaze_features=None, measured_gaze_feature_mask=None, scanpath_source="eyettention", return_debug_dict=False):
		...
		if measured_gaze_features is not None:
			measured_gaze_features = measured_gaze_features.view((-1, measured_gaze_features.size(-2), measured_gaze_features.size(-1))).to(input_ids.device).float()
		if measured_gaze_feature_mask is not None:
			measured_gaze_feature_mask = measured_gaze_feature_mask.view((-1, measured_gaze_feature_mask.size(-2), measured_gaze_feature_mask.size(-1))).to(input_ids.device).float()
		...
		if self.use_gaze_features and measured_gaze_features is not None:
			gaze_token_pos, sp_len, measured_gaze_features_token_level = self.convert_word_pos_seq_to_token_pos_seq(word_pos=gaze_pos,
										sn_len=sn_len,
										word_ids_sn=LM_word_ids,
										measured_gaze_features=measured_gaze_features,
										measured_gaze_feature_mask=measured_gaze_feature_mask)
		else:
			gaze_token_pos, sp_len = self.convert_word_pos_seq_to_token_pos_seq(word_pos=gaze_pos,
										sn_len=sn_len,
										word_ids_sn=LM_word_ids)
			measured_gaze_features_token_level = None
		...
		if return_debug_dict:
			...
			if measured_gaze_features_token_level is not None:
				debug_dict["measured_gaze_features_token_level_shape"] = tuple(measured_gaze_features_token_level.shape)
			return gaze_token_pos, sp_len, measured_gaze_features_token_level, debug_dict

		return gaze_token_pos, sp_len, measured_gaze_features_token_level

	def _write_gaze_fusion_debug(self, lines):
		if not self.debug_gaze_features or self._wrote_gaze_fusion_debug:
			return
		output_path = Path("Variante_A_metricas_rama_auxiliar/Pasos/paso_C_model_fusion/model_fusion_shapes.txt")
		output_path.parent.mkdir(parents=True, exist_ok=True)
		output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
		self._wrote_gaze_fusion_debug = True

	def forward(self, sp_pooler_output, input_ids, attention_mask, token_type_ids, word_ids, word_len, LM_word_ids, measured_word_ids=None, measured_sp_len=None, measured_gaze_features=None, measured_gaze_feature_mask=None, scanpath_source="eyettention", return_debug_dict=False):
		if return_debug_dict:
			gaze_token_pos, sp_len, measured_gaze_features_token_level, debug_dict = self.SP_Gen(
				input_ids,
				attention_mask,
				token_type_ids,
				word_ids,
			...
			gaze_token_pos, sp_len, measured_gaze_features_token_level = self.SP_Gen(
				input_ids,
				attention_mask,
				token_type_ids,
				word_ids,
		...
		x_sp_before_fusion_shape = tuple(x_sp.shape)
		fusion_debug = None
		if self.use_gaze_features and self.gaze_feature_fusion is not None and measured_gaze_features_token_level is not None:
			measured_gaze_features_token_level = measured_gaze_features_token_level.to(x_sp.device).to(x_sp.dtype)
			x_sp, fusion_debug = self.gaze_feature_fusion(
				x_sp,
				measured_gaze_features_token_level,
				return_debug_dict=(return_debug_dict or self.debug_gaze_features),
			)
		...
		if fusion_debug is not None:
			lines = [
				f"x_sp_before_fusion.shape: {x_sp_before_fusion_shape}",
				f"measured_gaze_features_token_level.shape: {tuple(measured_gaze_features_token_level.shape)}",
				f"gaze_proj.shape: {fusion_debug['gaze_proj_shape']}",
				f"gate.mean: {fusion_debug['gate_mean']}",
				f"gate.min: {fusion_debug['gate_min']}",
				f"gate.max: {fusion_debug['gate_max']}",
				f"x_sp_after_fusion.shape: {tuple(x_sp.shape)}",
				f"sp_len.shape: {tuple(sp_len.shape)}",
			]
			self._write_gaze_fusion_debug(lines)
		...
		if return_debug_dict:
			...
			if measured_gaze_features_token_level is not None:
				debug_dict["measured_gaze_features_token_level"] = measured_gaze_features_token_level.detach()
			if fusion_debug is not None:
				debug_dict["gaze_feature_fusion"] = fusion_debug
...
def orig_forward(orig_self,
	...
	pooled_output = orig_self.dropout(pooled_output)
	logits = orig_self.classifier(pooled_output)
...
def aug_forward(orig_self,
	...
	##compute L_scanpath
	# Pooling
	pooler_output = outputs.last_hidden_state # (bs, len, hidden)
	sp_pooler_output = orig_self.sp_encoder(
							sp_pooler_output=pooler_output,
							input_ids=ET_input_ids,
							attention_mask=ET_attention_mask,
							token_type_ids=ET_token_type_ids,
	...
	sp_pooled_output = orig_self.dropout(sp_pooled_output)
	sp_logits = orig_self.classifier(sp_pooled_output)
...
class Gazesup_BERTForSequenceClassification(BertPreTrainedModel):
	...
	def add_sp_func(self, config):
		...
		self.sp_encoder = SP_Encoder(
			config,
			scanpath_source=getattr(self.model_args, "scanpath_source", "eyettention"),
			use_gaze_features=getattr(self.model_args, "use_gaze_features", False),
			gaze_feature_dim=getattr(self.model_args, "gaze_feature_dim", 3),
			debug_gaze_features=getattr(self.model_args, "debug_gaze_features", False),
		)
...
class Eyettention_pretrain(nn.Module):
	def __init__(self, cf):
		...
		if self.cf["model_pretrained"].startswith('RoBERTa'):
			self.bert = RobertaModel.from_pretrained(self.cf["model_pretrained"], config = bert_encoder_config)
